...
const RATE = 200;
const CLOUD_MODE = Boolean(window.google?.script?.run);
...
function loadState() {
  try {
    ...
    if (saved && Array.isArray(saved.students) && Array.isArray(saved.sessions)) return migrateState(saved);
  } catch (error) {
...
function migrateState(saved) {
  if (!saved || !Array.isArray(saved.students) || !Array.isArray(saved.sessions)) throw new Error("This is not a valid TutoringTracker backup");
  const now = new Date();
  ...
  return {
    ...
    sessions: saved.sessions.map((session) => ({
      ...session,
      status: validStatuses.has(session.status) ? session.status : session.date < todayKey ? "completed" : "scheduled",
      prepaidApplied: Number(session.prepaidApplied || 0)
    })),
...
function persist(message) {
  ...
  if (CLOUD_MODE && cloudReady) queueCloudSave();
...
function renderCalendar() {
  ...
  const projectedSessions = monthlySessions.filter((session) => session.status !== "cancelled");
  const earned = completedSessions.reduce((sum, session) => sum + Number(session.fee), 0);
  const projected = projectedSessions.reduce((sum, session) => sum + Number(session.fee), 0);
  const outstanding = completedSessions.filter((session) => session.payment === "unpaid").reduce((sum, session) => sum + Number(session.fee), 0);
  const hours = completedSessions.reduce((sum, session) => sum + Number(session.duration), 0);
  $("#hoursTotal").textContent = hours % 1 === 0 ? hours : hours.toFixed(1);
  $("#earnedTotal").textContent = money(earned);
  $("#projectedTotal").textContent = money(projected);
...
function sessionBar(session) {
  ...
  const statusLabels = { scheduled: "Upcoming", completed: "Completed", cancelled: "Cancelled" };
  const statusMarks = { scheduled: "&#9716;", completed: "&#10003;", cancelled: "&times;" };
  const paymentLabel = session.payment === "unpaid" ? "Still to pay" : session.payment === "prepaid" ? "Covered by prepaid credit" : "Paid";
  return `<button class="session-bar ${subjectClass} ${session.status}" type="button" data-session-id="${session.id}" title="${statusLabels[session.status]} · Edit ${escapeHtml(student?.name || "Deleted student")}'s session">
    ...
    <span class="session-signals"><i class="lesson-state ${session.status}" aria-label="${statusLabels[session.status]}">${statusMarks[session.status]}</i><i class="session-status ${session.payment}" aria-label="${paymentLabel}"></i></span>
  </button>`;
}
...
function renderPayments() {
  ...
  const unpaid = completed.filter((session) => session.payment === "unpaid").sort((a, b) => a.date.localeCompare(b.date));
  const paidTotal = completed.filter((session) => session.payment !== "unpaid").reduce((sum, session) => sum + Number(session.fee), 0);
  ...
  $("#paymentsSummary").innerHTML = `<div><span>Completed value</span><strong>${money(paidTotal + unpaidTotal)}</strong></div><div><span>Covered / paid</span><strong>${money(paidTotal)}</strong></div><div><span>Outstanding</span><strong>${money(unpaidTotal)}</strong></div><div><span>Credit held</span><strong>${money(creditHeld)}</strong></div>`;
  ...
  if (!unpaid.length) {
    list.innerHTML = `<div class="empty-state"><strong>You’re all settled</strong>There are no unpaid completed lessons at the moment.</div>`;
    return;
  }
...
function openSessionModal(date = isoDate(new Date()), studentId = "", sessionId = "") {
  ...
  $("#sessionStatus").value = existing?.status || defaultStatusForDate(existing?.date || date);
  ...
  $("#completeSessionButton").classList.toggle("hidden", !existing || existing.status !== "scheduled");
...
function saveSession(event) {
  ...
  if (existing) {
    ...
    if (baseSession.status !== "cancelled" && baseSession.payment === "prepaid" && Number(student.creditBalance) + refundable < baseSession.fee) {
    ...
    if (!repeat && baseSession.status !== "cancelled" && baseSession.payment === "prepaid" && Number(student.creditBalance) < baseSession.fee) {
    ...
    uniqueDates.forEach((date) => {
      const session = { ...baseSession, id: uid("ses"), date, seriesId, status: repeat ? defaultStatusForDate(date) : baseSession.status };
...
function applyPayment(session, allowPrepaidFallback) {
  if (session.payment !== "prepaid" || session.status === "cancelled") return false;
  const student = studentFor(session.studentId);
...
function defaultStatusForDate(dateString) {
  return dateString < isoDate(new Date()) ? "completed" : "scheduled";
}
...
function queueCloudSave() {
  ...
  cloudPending = true;
  setSaveStatus("Saving to Google…", "saving");
  cloudSaveTimer = setTimeout(flushCloudSave, 400);
}

function flushCloudSave() {
  if (cloudSaving || !cloudPending) return;
  cloudSaving = true;
  cloudPending = false;
  google.script.run
      .withSuccessHandler((result) => {
        ...
        cloudSaving = false;
        setSaveStatus("Synced to Google");
        if (cloudPending) flushCloudSave();
      })
      .withFailureHandler((error) => {
        console.error("Cloud save failed", error);
        cloudSaving = false;
        setSaveStatus("Cloud sync needs attention", "error");
        showToast(error?.message || "Cloud save failed — your device copy is still safe");
      })
      .saveCloudState(structuredClone(state), cloudRevision);
}

function hydrateCloudState() {
  setSaveStatus("Loading from Google…", "saving");
  google.script.run
    .withSuccessHandler((result) => {
      try {
        state = migrateState(result?.state || defaultState);
        ...
        setSaveStatus("Cloud data needs attention", "error");
      }
    })
    .withFailureHandler((error) => {
      console.error("Cloud load failed", error);
      setSaveStatus("Offline device copy", "error");
      showToast("Could not reach Google — showing this device’s copy");
    })
    .getCloudState();
}

function exportBackup() {
  const payload = JSON.stringify({ format: "TutoringTrackerBackup", version: 1, exportedAt: new Date().toISOString(), state }, null, 2);
  const url = URL.createObjectURL(new Blob([payload], { type: "application/json" }));
  const link = document.createElement("a");
  link.href = url;
  link.download = `TutoringTracker-backup-${isoDate(new Date())}.json`;
...
async function importBackup(file) {
  try {
    ...
    if (!window.confirm(`Import ${imported.students.length} students and ${imported.sessions.length} sessions? This replaces the records currently shown here.`)) return;
    state = imported;
    persist("Backup imported and saved");
    renderAll();
    closeDrawer();
  } catch (error) {
...
$("#sessionDate").addEventListener("change", (event) => {
  syncRepeatUntil();
  if (!$("#sessionId").value) $("#sessionStatus").value = defaultStatusForDate(event.target.value);
});
...
renderAll();
if (CLOUD_MODE) hydrateCloudState();
